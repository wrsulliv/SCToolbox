function error = RunThisMultipleInputDynamicRange(a, order, complete_a, root_number);
    ...
    complete_z_new = complete_z_new .^ root_number;
    ...
    subplot(1, 2, 1);
    scatter(complete_x(:, 1), complete_z);
    %axis([0 1 0 2])
    subplot(1, 2, 2);
    scatter(complete_x(:, 1), complete_z_new);
    %axis([0 1 0 2])
    %subplot(2, 2, 1);
    %scatter(complete_x(:, 1), complete_z);
    %axis([-0.5 1 0 2])
    ...
    %axis([-0.5 1 0 2])
    %subplot(2, 2, 3);
    %scatter(complete_x(:, 2), complete_z);
    %axis([-0.5 1 0 2])
    %subplot(2, 2, 4);
    %scatter(complete_x(:, 2), complete_z_new);
    %axis([-0.5 1 0 2])
    ...
    fprintf(fp, '%f\n%d\n%d\n', error, length(input_offset), root_number);
