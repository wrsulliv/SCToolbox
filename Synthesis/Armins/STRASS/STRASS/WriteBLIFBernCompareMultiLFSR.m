function Y = WriteBLIFBernCompareMultiLFSR(TT, noOfX, noOfR, filename, modelname);
	...
	for i=1:ll
		...
		if(temp != 2^noOfR)
			for j=noOfR-1:-1:0
				if(j!=0)
					fprintf(fp, '\n\n.names');
					fprintf(fp, ' c_%d_%d r%d_%d carry_%d_%d', i, j, j+1, i, i, j);
					fprintf(fp, ' carry_%d_%d\n', i, j+1);
					fprintf(fp, '10- 1\n001 1\n111 1\n');
				else
					fprintf(fp, '\n\n.names');
					fprintf(fp, ' c_%d_%d r%d_%d', i, j, j+1, i);
					fprintf(fp, ' carry_%d_%d\n', i, j+1);
					fprintf(fp, '10 1\n');
				end
			end
