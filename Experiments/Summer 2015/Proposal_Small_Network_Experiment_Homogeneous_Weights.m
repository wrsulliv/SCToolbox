...
Explanation
    ...
    initialized randomly and homogeneously and the inputs are from the Iris 
    dataset.  Homogeneous weights are weights which are the same for all
    neurons. The FSM size of each neuron is determined by the minimum 
    of all the weights
...
bitstreamLength = 1000;
randomWeightScaleFactor = 10;
...
for numLayers = 1:1:numSimulatedLayers
    numLayers
   ...
   % w_ho_dec = 0 + rand(numNodesPerLayer+1, numNodesPerLayer, numLayers-1)/10;
    w_ho_sc = zeros(bitstreamLength, numNodesPerLayer+1, numNodesPerLayer, numLayers-1);
    ...
    FSM_STATES_I = floor(2*numInputsFromInput*max(w_all(:, 1)));
    FSM_STATES_HO = floor(2*numInputsFromHidden*max(w_all(1:3, 1)));
    AVG_STEPS = 1;
    ...
    for i = 1:sample_count
        ...
        for layer = 1:numLayers
            for node = 1:numNodesPerLayer
                ...
                elseif(layer == numLayers)
                     % The last layer uses linear neurons
                    current_output_normal(node, :) = NEURON(current_input_normal, w_ho_sc(:, :, node, layer-1)', FSM_STATES_HO, false, true);
                    current_output_regen(node, :) = NEURON(current_input_regen, w_ho_sc(:, :, node, layer-1)', FSM_STATES_HO, false, true);
                    current_output_delay(node, :) = NEURON(current_input_delay, w_ho_sc(:, :, node, layer-1)', FSM_STATES_HO, false, true);
