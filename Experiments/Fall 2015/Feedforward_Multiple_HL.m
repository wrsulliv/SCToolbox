...
data = csvread('glass.csv'); % A (numSamples) x (numFeatures + 1) matrix
numClasses = 7;
...
numTrain = floor(numSamples*0.95);
...
%                     network_percent_error_map(l, k) = network_percent_error_map(l, k) + neuron_percent_error_hidden;
    ...
    sc_error = sum(sum(abs((results - t_test')), 2) > 0) / length(t_test);
    %sc_error(hiddenLayerBreadth, hiddenLayerDepth) = my_error;
    %debug_error(hiddenLayerBreadth, hiddenLayerDepth) = current_debug_error;
    
    output = sim(net, phi_test');
    output = (output - ones(numClasses, 1)*max(output)) >= 0;
    software_error = sum(sum(abs((output' - t_test')), 2) > 0) / length(t_test)
